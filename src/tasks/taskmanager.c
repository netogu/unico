...
TaskHandle_t task_manager_init(void) {
  ...
  task_manager_handle = xTaskCreateStatic(
      task_manager, xstr(TASK_NAME_TASK_MANAGER), TASK_STACK_SIZE_TASK_MANAGER,
      NULL, TASK_PRIORITY_TASK_MANAGER, task_manager_stack, &task_manager_tcb);
...
static void task_manager(void *parameters) {
  ...
  vTaskDelay(150); // Wait for Shell to catch up
  board_hw_setup();
  task_msg_init();
  vTaskDelay(100);
